...
#include <opencv2/imgcodecs.hpp>  // ✅ ADD THIS LINE for cv::imwrite
...
bool SVStitcher::init(const std::vector<cv::cuda::GpuMat>& imgs){
    ...
    // ✅ DEBUG: Save seam masks
    for (size_t i = 0; i < gpu_seam_masks.size(); ++i) {
        cv::Mat mask_cpu;
        gpu_seam_masks[i].download(mask_cpu);
        cv::imwrite("debug_seam_mask_" + std::to_string(i) + ".png", mask_cpu);
        std::cout << "Saved seam mask " << i << std::endl;
    }
