...
bool SVDisplayView::render(const cv::cuda::GpuMat& frame)
{
    if (!glfwWindowShouldClose(window) && isInit) {
        ...
        // ✅ COMMENT OUT THIS LINE to stop continuous rotation
        //if (useDemoTopView)
        //    demoTVMode(cam);
...
{
//=============Old code for rotations===========
    //useDemoTopView = topview;
    //resetCameraState();
    //cam.setCamPos(glm::vec3(0, 4.15, 0.0));
    
//=============updated code for stop rotations===========
    if (topview) {
        // Set camera to look straight down from above
        resetCameraState();
        cam.setCamPos(glm::vec3(0, 4.15, 0.0));  // Position above the scene
        cam.setCamPitch(-89.0f);  // Look straight down
        cam.setCamYaw(-90.0f);    // Face forward
    }
